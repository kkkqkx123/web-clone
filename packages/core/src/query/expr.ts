/**
 * @web-clone/core — Safe expression language for --where filtering
 *
 * A tiny, safe expression language. No eval, no side effects.
 *
 *   price > 100 && stock != 0
...
function lex(src: string): Tok[] {
  ...
  while (i < src.length) {
    ...
    if (c === '/' /* regex literal */) {
      ...
      if (end === -1) throw new QueryError(`unterminated regex in expression: ${src.slice(i)}`);
      const flagsMatch = /^[a-z]*/.exec(src.slice(end + 1));
      const flags = flagsMatch?.[0] ?? '';
      try {
        toks.push({ t: 're', v: new RegExp(src.slice(i + 1, end), flags) });
      } catch (e) {
        throw new QueryError(`invalid regex: ${(e as Error).message}`);
      }
    ...
    if (c === '"' || c === "'") {
      ...
      if (end === -1) throw new QueryError(`unterminated string in expression: ${src.slice(i)}`);
    ...
    if (c === '`' /* quoted column name — for headers with spaces */) {
      const end = src.indexOf('`', i + 1);
      if (end === -1) throw new QueryError(`unterminated \`column name\` in expression: ${src.slice(i)}`);
    ...
    throw new QueryError(`cannot parse expression near: ${src.slice(i, i + 20)}`);
  }
  return toks;
}
...
function parse(toks: Tok[]): Node {
  ...
  function primary(): Node {
    const t = eat();
    if (!t) throw new QueryError('unexpected end of expression');
    if (t.t === 'lp') {
      const e = or();
      const close = eat();
      if (!close || close.t !== 'rp') throw new QueryError('missing ) in expression');
      return e;
    }
    ...
    throw new QueryError('unexpected token in expression');
  }
  ...
  const root = or();
  if (pos !== toks.length) throw new QueryError('trailing tokens in expression');
  return root;
}
      ...
      switch (n.op) {
        ...
        case '~':
          return r instanceof RegExp
            ? r.test(String(l ?? ''))
            : (() => { throw new QueryError('~ needs a /regex/ on the right'); })();
        case '!~':
          return r instanceof RegExp
            ? !r.test(String(l ?? ''))
            : (() => { throw new QueryError('!~ needs a /regex/ on the right'); })();
...
/**
 * Compile a `--where` expression string into a predicate function.
 *
 * The returned function takes a context object and returns a boolean.
 * It throws `QueryError` on type mismatches or invalid operations.
 */
