/**
 * Puppeteer Integrated Test Workflow — inspect + snapshot + assertions
 *
 * Demonstrates a complete E2E test workflow that combines:
 *   1. Puppeteer — browser automation (interaction, JS rendering)
 ...
 *   - Cookies are page-level (not context-level like Playwright)
 *   - Sub-resources are fetched via raw HTTP with cookie forwarding
 *   - getAuthContext() extracts cookies + localStorage from the current page
 *
 * Usage:
 *   pnpm tsx examples/puppeteer/04-integrated-test-workflow.ts
 *
 *   # Simulate a failure:
 *   ASSERT_FAIL=true pnpm tsx examples/puppeteer/04-integrated-test-workflow.ts
 *
 * Prerequisites:
 *   pnpm add @web-clone/core @web-clone/adapter-puppeteer puppeteer jsdom
 */
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      // Assertion 4: Search for expected text
      ...
      // ═══════════════════════════════════════════════════════
      // Phase 4: Handle failures — save artifacts + snapshot
      // ═══════════════════════════════════════════════════════
      ...
      if (failedSteps.length > 0) {
        ...
        // Optional: Save full snapshot with assets
        if (RUN_SNAPSHOT) {
